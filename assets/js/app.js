(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        function addEllipsisToFixedHeight(selector, container = null) {
            ...
            elements.forEach((el => {
                ...
                if (el.scrollHeight > maxHeight) {
                    let trimmedText = el.textContent;
                    let start = 0;
                    let end = trimmedText.length;
                    let mid;
                    let lastValidText = trimmedText;
                    while (start <= end) {
                        mid = Math.floor((start + end) / 2);
                        el.textContent = trimmedText.slice(0, mid).trim() + "...";
                        if (el.scrollHeight > maxHeight) end = mid - 1; else {
                            start = mid + 1;
                            lastValidText = el.textContent;
                        }
                    }
        ...
        addEllipsisToFixedHeight("._multiline-shorten");
        const truncate = (text, limit) => text.substr(0, limit) + "...";
        const toggleText = (item, maxChars) => {
            const text = item.querySelector("div");
            let newText = "";
            if (!item.dataset.text) item.dataset.text = text.innerHTML;
            newText = item.classList.contains("active") ? item.dataset.text : truncate(item.dataset.text, maxChars || item.dataset.text.length / 1.5);
            text.innerHTML = newText;
            item.classList.toggle("active");
        };
        document.querySelectorAll("[data-truncate-block]").forEach((item => {
            const button = item.querySelector("button");
            const buttonText = button.querySelector("span");
            const maxChars = parseInt(item.dataset.truncateBlock);
            toggleText(item, maxChars);
            button.addEventListener("click", (function() {
                buttonText.textContent = buttonText.textContent === "Открыть полностью" ? "Свернуть отзыв" : "Открыть полностью";
                toggleText(item);
            }));
        }));
        const toggleButtons = document.querySelectorAll("[data-show-more]");
        if (toggleButtons.length > 0) toggleButtons.forEach((button => {
            const target = document.querySelector(`[data-show-block="${button.dataset.showMore}"]`);
            const hiddenItems = target?.querySelectorAll("[hidden]");
            if (!target || hiddenItems.length === 0) {
                button.remove();
                return;
            }
            if (target && hiddenItems.length > 0) {
                let openedItems = 0;
                button.addEventListener("click", (function() {
                    if (openedItems < hiddenItems.length) {
                        for (let i = openedItems; i < openedItems + 3 && i < hiddenItems.length; i++) {
                            _slideDown(hiddenItems[i]);
                            addEllipsisToFixedHeight("._multiline-shorten", hiddenItems[i]);
                        }
                        openedItems += 3;
                    }
                    if (openedItems >= hiddenItems.length) button.remove();
                }));
            }
        }));
        ...
        if (fromPrice && toPrice) {
            fromPrice.addEventListener("change", (function() {
                let fromPriceValue = parseInt(fromPrice.value);
                let toPriceValue = parseInt(toPrice.value);
                if (fromPriceValue > toPriceValue) toPrice.value = fromPriceValue + 1;
            }));
            toPrice.addEventListener("change", (function() {
                let fromPriceValue = parseInt(fromPrice.value);
                let toPriceValue = parseInt(toPrice.value);
                if (toPriceValue < fromPriceValue) fromPrice.value = toPriceValue - 1;
            }));
        }
        window["FLS"] = true;
    })();
